...
<div class="w-full min-h-screen rounded-tl-xl px-10">
    ...
    <form method="GET"
        ...
        <div>
            ...
            <select name="anio" class="rounded-lg border-gray-300 focus:ring-2 focus:ring-[#2E6C6F]">
        ...
        <div>
            ...
            <select name="mes" class="rounded-lg border-gray-300 focus:ring-2 focus:ring-[#2E6C6F]">
                <option value="">Todos</option>
        ...
        <div>
            ...
            <select name="propiedad_id" class="rounded-lg border-gray-300 focus:ring-2 focus:ring-[#2E6C6F]">
        ...
        <div>
            ...
            <select name="estado" class="rounded-lg border-gray-300 focus:ring-2 focus:ring-[#2E6C6F]">
                <option value="">Todos</option>
        ...
        <table class="min-w-full text-sm text-gray-700">
            ...
            <tbody class="divide-y">
                ...
                <tr class="hover:bg-[#F3FAFA] transition">
                    <td class="px-6 py-4 text-center">{{ $pago->propiedad->nombre }}</td>
                    ...
                    <td class="px-6 py-4 text-center">${{ number_format($pago->monto,0,',','.') }}</td>
                    <td class="px-6 py-4 text-center">
                        ...
                        <div x-data="{ open: false, mensaje: '' }">
                            ...
                            >
                                <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                                    ...
                                    <div class="mt-1 mb-5 inline-block px-4 py-1 bg-[#2C7474]/10 text-[#2C7474] rounded-full text-sm font-medium text-center">
                                        {{ $pago->fecha_inicio->format('d/m/Y') }} —
                                        {{ $pago->fecha_fin->format('d/m/Y') }}
                                    </div>

                                    <!-- FORMULARIO -->
                                    <form method="POST" action="{{ route('pagos.marcar-pagado', $pago->id) }}" class="space-y-4">
                                        @csrf

                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Número de tarjeta</label>
                                            <input type="text" placeholder="1234123412341234" class="mt-1 w-full border rounded-xl p-3">
                                        </div>

                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Nombre del titular</label>
                                            <input type="text" placeholder="Juan Pérez" class="mt-1 w-full border rounded-xl p-3">
                                        </div>

                                        <div class="flex gap-4">
                                            <div class="w-1/2">
                                                <label class="block text-sm font-medium text-gray-700">Mes</label>
                                                <input type="text" placeholder="MM" class="mt-1 w-full border rounded-xl p-3">
                                            </div>
                                            <div class="w-1/2">
                                                <label class="block text-sm font-medium text-gray-700">Año</label>
                                                <input type="text" placeholder="YY" class="mt-1 w-full border rounded-xl p-3">
                                            </div>
                                        </div>

                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">CVV</label>
                                            <input type="text" placeholder="123" class="mt-1 w-full border rounded-xl p-3">
                                        </div>

                                        <div class="flex justify-between mt-4">
                                            <button type="button" @click="open = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 transition">
                                                Cerrar
                                            </button>

                                            <button type="submit" class="px-4 py-2 bg-[#2C7474] text-white rounded-xl hover:bg-[#245f5f] transition">
                                                Confirmar pago
                                            </button>
                                        </div>
    ...
    <div class="md:hidden space-y-4">
        ...
        <div class="bg-white rounded-2xl shadow p-4 space-y-3">
            ...
            <div>
                ...
                <p>{{ ucfirst(\Carbon\Carbon::create()->month($pago->mes)->locale('es')->monthName) }} {{ $pago->anio }}</p>
            </div>

            <div>
                <p class="text-xs text-gray-500">Monto</p>
                <p class="font-semibold">${{ number_format($pago->monto,0,',','.') }}</p>
            </div>

            <div>
                <p class="text-xs text-gray-500">Estado</p>
                <span class="rounded-full text-xs font-medium
                    {{ $pago->estado === 'pagado' ? 'text-green-500' : 'text-yellow-500' }}">
                    {{ ucfirst($pago->estado) }}
                </span>
            </div>
            ...
            <div x-data="{ open: false }">
                <button @click="open = true" class="px-4 py-2 bg-[#2C7474] text-white rounded-xl hover:bg-[#245f5f] transition cursor-pointer">
                    Pagar
                </button>
